...
def test(env, actor_model):
    ...
    policy = NetActor(obs_dim, act_dim).to(device)
...
def evaluate(env, hyperparameters, actor_model, critic_model, num_episodes):
    ...
    for ep in range(num_episodes):
        ...
        obs = env.reset()
        navigator = None
        if hyperparameters.get('hybrid_residual', False):
        ...
        while ep_length < hyperparameters['max_timesteps_per_episode']:
            ...
            with torch.no_grad():
                ...
                residual_action = policy(obs_tensor).cpu().numpy()
                residual_action = np.clip(residual_action, -1.0, 1.0)
                if navigator is not None:
                    base_action, safety_locked = navigator.base_action(env)
                    action = base_action if safety_locked else navigator.compose(base_action, residual_action)
                else:
...
def main(args):
    ...
    if args.hybrid_residual:
        print('[main] Hybrid residual PPO: enabled', flush=True)
    """
