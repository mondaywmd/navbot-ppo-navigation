...
class SafeDetourExpert:
    ...
    def action(self, env):
        ...
        passed = float(env.position.x) > self.scene["pillar_x"] + 0.38
        if passed:
            waypoint = (2.0, self.scene["target_y"])
            phase = "recover"
        else:
            waypoint = (self.scene["pillar_x"] + 0.18,
                        self.scene["pillar_y"] + self.scene["detour_sign"] * 0.68)
            phase = "detour"
        ...
        linear = -0.2 if abs(error) > math.radians(55) else 0.6
