...
def main():
    root=os.path.dirname(os.path.abspath(__file__));outdir=os.path.join(root,"results","multilevel_expert_batch_seed8042704")
    ...
    try:
        for level in (1,2,3):
            for base in generate(level,5,seed=8042704):
                for scene in (base,mirror(base)):
                    row=run(env,scene);rows.append(row);print("MULTI_BATCH level=%d name=%s side=%s outcome=%s steps=%d min=%.3f"%(level,row["name"],row["side"],row["outcome"],row["steps"],row["min_lidar"]),flush=True)
        ...
        with open(os.path.join(outdir,"summary.json"),"w")as f:json.dump({"seed":8042704,"episodes":rows,"successes":sum(r["outcome"]=="success"for r in rows),"collisions":sum(r["outcome"]=="collision"for r in rows),"timeouts":sum(r["outcome"]=="timeout"for r in rows),"training":False,"dataset_created":False},f,indent=2,sort_keys=True)
