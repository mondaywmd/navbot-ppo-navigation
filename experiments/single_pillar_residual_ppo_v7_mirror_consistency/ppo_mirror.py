...
class PPO:
    ...
    def __init__(self, policy_class, value_func, env, state_dim, action_dim, **hyperparameters):
        ...
        self.cov_var = torch.full(size=(self.act_dim,), fill_value=0.05).to(device)
    ...
    def learn(self, total_timesteps, past_action):
        ...
        while t_so_far < total_timesteps:  # ALG STEP 2
            ...
            batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens, iter_metrics, batch_vision_feats, batch_residual_active = self.rollout(past_action=past_action, t_so_far=t_so_far)  # ALG STEP 3
            ...
            # Calculate advantage at k-th iteration.  Optional per-batch
            # return normalization keeps the critic target near unit scale;
            # advantages are normalized again below for the policy update.
            raw_batch_rtgs = batch_rtgs
            if self.normalize_returns:
                return_mean = raw_batch_rtgs.mean()
                return_std = raw_batch_rtgs.std(unbiased=False)
                critic_targets = (raw_batch_rtgs - return_mean) / (return_std + 1e-8)
            else:
                critic_targets = raw_batch_rtgs
            self.V, _ = self.evaluate(batch_obs, batch_acts, batch_vision_feats)
            value_func.append(self.V.detach().mean())
            with torch.no_grad():
                target_variance = torch.var(critic_targets, unbiased=False)
                error_variance = torch.var(critic_targets - self.V.detach(), unbiased=False)
                explained_variance = (
                    1.0 - error_variance / target_variance
                    if target_variance > 1e-12 else torch.tensor(0.0, device=device)
                )
                self.logger['critic_scale'] = {
                    'raw_target_mean': raw_batch_rtgs.mean().item(),
                    'raw_target_std': raw_batch_rtgs.std(unbiased=False).item(),
                    'target_mean': critic_targets.mean().item(),
                    'target_std': critic_targets.std(unbiased=False).item(),
                    'target_min': critic_targets.min().item(),
                    'target_max': critic_targets.max().item(),
                    'value_mean': self.V.detach().mean().item(),
                    'value_std': self.V.detach().std(unbiased=False).item(),
                    'explained_variance': explained_variance.item(),
                }
            A_k = critic_targets - self.V.detach()  # ALG STEP 5
            ...
            actor_active_mask = batch_residual_active > 0.5
            actor_active_count = int(actor_active_mask.sum().item())
            if self.hybrid_residual:
                print(f"[Hybrid PPO] Actor residual-active steps: {actor_active_count}/{len(actor_active_mask)}", flush=True)
            ...
            for _ in range(self.n_updates_per_iteration):  # ALG STEP 6 & 7
                ...
                with torch.no_grad():
                    ...
                    # Entropy (policy)
                    # Entropy is over the 2-D action distribution, independent
                    # of the number of transitions in this batch.  Using a
                    # zeros_like(ratios) location accidentally treated batch
                    # length as the event dimension and produced huge values.
                    entropy_dist = MultivariateNormal(
                        torch.zeros(self.act_dim, device=device), self.cov_mat
                    )
                    entropy = entropy_dist.entropy().item()
                ...
                if actor_active_count > 0:
                    ppo_actor_loss = (-torch.min(surr1[actor_active_mask], surr2[actor_active_mask])).mean()
                else:
                    # Keep a valid zero-gradient graph if safety owned every action.
                    ppo_actor_loss = (curr_log_probs * 0.0).mean()
                mirror_loss, mirror_parts = actor_mirror_consistency_loss(self.actor, batch_obs)
                actor_loss = ppo_actor_loss + self.mirror_loss_coef * mirror_loss
                if _ == 0:
                    print(f"[V7 Mirror] coef={self.mirror_loss_coef:.4f} ppo={ppo_actor_loss.item():.6f} mirror={mirror_loss.item():.6f} linear={mirror_parts['linear_mse'].item():.6f} angular={mirror_parts['angular_mse'].item():.6f}", flush=True)
                critic_loss = nn.MSELoss()(self.V, critic_targets)
    ...
    def rollout(self, past_action, t_so_far):
        ...
        navigator = None
        if self.hybrid_residual:
            from hybrid_navigation import HybridNavigator
            navigator = HybridNavigator()
            navigator.reset()
            print("[Hybrid PPO] LiDAR safety base enabled.", flush=True)
        done = False
        episode_reward = 0
        one_round = 0
        ep_rews = []
        ...
        # Rollout loop: collect at least one full batch, then finish
        # the active episode so metrics never contain a truncated fragment.
        t = 0
        while True:
            ...
            if self.hybrid_residual:
                base_action, safety_locked = navigator.base_action(self.env)
                executed_action = (
                    base_action
                    if safety_locked
                    else navigator.compose(base_action, action)
                )
                residual_active = not safety_locked
            else:
            ...
            # Step environment
            obs, rew, done, arrive = self.env.step(executed_action, past_action)

            past_action = executed_action
            episode_reward += rew
            ep_rews.append(rew)
            batch_acts.append(action)
            batch_residual_active.append(float(residual_active))
            ...
            if timeout and not done and not arrive:
                timeout_penalty = -150.0
                episode_reward += timeout_penalty
                ep_rews[-1] += timeout_penalty
            ...
            if done or arrive or timeout:
                ...
                # Do not cut an episode in half at the batch boundary.
                if (t + 1) >= self.timesteps_per_batch:
                    break
        ...
        # Record a rollout fragment that reaches the batch boundary.
        # This keeps batch_lens aligned with the 400 collected transitions.
        if one_round != 0:
            batch_lens.append(one_round)
            batch_rews.append(ep_rews)
        # if one_round != 0:
        ...
        batch_residual_active = torch.from_numpy(np.array(batch_residual_active, dtype=np.float32)).float()
        ...
        return (
            batch_obs.to(device), batch_acts.to(device), batch_log_probs.to(device),
            batch_rtgs.to(device), batch_lens, iter_metrics,
            batch_vision_feats.to(device) if batch_vision_feats is not None else None,
            batch_residual_active.to(device),
        )
    ...
    def get_action(self, obs, t_so_far, one_round, vision_feat=None):
        ...
        if self.t_step == 0 and t_so_far > 0 and self.cov_mat[0][0] >= 0.02:
            self.cov_mat *= 0.995
        dist = MultivariateNormal(mean, self.cov_mat)

        action = dist.sample()
        # Clamp without in-place operations
        residual_action_mode = self.hybrid_residual or self.native_residual
        action_clamped = torch.stack([
            torch.clamp(action[:, 0], -1 if residual_action_mode else 0, 1),
    ...
    def evaluate(self, batch_obs, batch_acts, batch_vision_feats=None):
        ...
        mean_clamped = torch.stack([
            torch.clamp(mean[:, 0], -1 if residual_action_mode else 0, 1),
    ...
    def _init_hyperparameters(self, hyperparameters):
        ...
        self.hybrid_residual = False  # Rule base + PPO residual correction.
        self.native_residual = False  # Environment composes PPO residual with its base controller.
        self.normalize_returns = False  # Opt-in stable critic target scaling.
        self.mirror_loss_coef = 0.0  # V7 opt-in Actor equivariance constraint.
        ...
        conf = {
            ...
            'hybrid_residual': self.hybrid_residual,
            'native_residual': self.native_residual,
            'normalize_returns': self.normalize_returns,
            'mirror_loss_coef': self.mirror_loss_coef,
        }
    ...
    def _log_summary(self):
        ...
        critic_scale = self.logger.get('critic_scale', {})
        ...
        print(
            "Critic Scale: target mean={:.3f} std={:.3f} range=[{:.3f}, {:.3f}] | "
            "value mean={:.3f} std={:.3f} | explained_var={:.4f}".format(
                critic_scale.get('target_mean', 0.0), critic_scale.get('target_std', 0.0),
                critic_scale.get('target_min', 0.0), critic_scale.get('target_max', 0.0),
                critic_scale.get('value_mean', 0.0), critic_scale.get('value_std', 0.0),
                critic_scale.get('explained_variance', 0.0),
            ),
            flush=True,
        )
        print(f"================================================================================", flush=True)
        ...
        for name, value in critic_scale.items():
            self.writer.add_scalar("critic/" + name, value, i_so_far)
